  ...
  #sale
    ...
    .outer-box#hisSaleOrder
      ...
      table.production-table.f-m-t-10
        thead
          tr
            th(colspan="4") 经销商名称: {{ saleData.orderForm.dealerName }}
            th(colspan="5") 地址: {{ saleData.orderForm.dealerAddress }}
            th(colspan="3") 电话: {{ saleData.orderForm.call }}
          tr
            th(colspan="4") 客户名称：{{ saleData.orderForm.clientName }}
        ...
        tbody
          ...
          tr
            td(colspan="8")
            td.f-fwb(colspan="4") 合计金额：￥ {{ saleData.totalPrice | formatMoney }}
          tr 
            td(colspan="12") 备注：{{ saleData.orderForm.orderNote }}
          template(v-if="saleData.detailForm")
            tr
              td(colspan="5") 应付金额：{{ saleData.detailForm.yingPay }}
              td(colspan="7") 实付金额：{{ saleData.detailForm.amountPay }}
            tr
              td(colspan="4") 销售单位：{{ saleData.detailForm.saleCompany }}
            ...
            tr
              td(colspan="12") 收货人：{{ saleData.detailForm.receiveName }}
</template>
